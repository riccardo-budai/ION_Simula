...
STIM_RATE = 20
N_PRE_STIM_SAMPLES = 20  # <--- Samples before stimulus
N_SIGNAL_SAMPLES = 200  # <--- Duration of SEP signal (formerly N_POINTS_PER_SWEEP)
N_POST_STIM_SAMPLES = 60  # <--- Samples after signal
...
class SimulationWindow(QMainWindow):
    ...
    def save_results_automatically(self):
        ...
        fileName = "../Virtual_patient/waterfall_averages.csv"  # Hardcoded filename
        try:
            # Transpose the list of 1D arrays into columns
            # Shape becomes (n_samples, n_blocks)
            data_to_save = np.array(self.waterfall_averages_to_save).T
    ...
    def load_results_from_csv(self):
        ...
        #                                           "CSV Files (*.csv)")
        # overwritten file
        fileName = "../Virtual_patient/waterfall_averages.csv"
        if fileName:
            try:
                # Load the data
                df = pd.read_csv(fileName)
